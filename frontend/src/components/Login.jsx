...
const Login = () => {
  ...
  const handleRegisterNavigate = () => {
    navigate(usertype === "education" ? "/register" : "/register");
  };
  ...
  const handleLogin = async (e) => {
    ...
    // Basic email validation (you can enhance this with more rules)
    ...
    try {
      // Sending the correct usertype along with email and password
      const { data } = await axios.post("http://localhost:8001/user/login", {
        email,
        password,
        usertype, // Make sure the correct usertype is sent here
      });
      ...
      if (data.success) {
        ...
        if (usertype === "education") {
          ...
          navigate("/healthcare-dashboard"); // Correct dashboard for healthcare
        } else {
  ...
  return (
    <div
      className="relative lg:mt-25 mt-25 min-h-screen bg-cover bg-center"
      ...
      >
        ...
        <form onSubmit={handleLogin}>
          <div className="mb-4 mt-2">
            <label htmlFor="email" className="block py-2 text-gray-700 text-xs font-bold">
              Email
            </label>
            <input
              type="email"
          ...
          <div className="mb-4 mt-2">
            <label htmlFor="password" className="block py-2 text-gray-700 text-xs font-bold">
              Password
            </label>
            <input
              type="password"
        ...
        {/* Render Register Now only if usertype is not admin */}
        {usertype !== "admin" && (
          <div className="mt-3 text-center">
